...
def change_sq_colour(window, sq):                           # change colour of squares on-click
    row, col = sq[0], sq[1]
    square = window.find_element(key=(row, col))
...
def update_board(window, dark, light, game_board):
    ...
    for x in range(8):
        for y in range(8):
            ...
            element = window.find_element(key=(x, y))
        ...
        if button == 'Select Board Colours':
            ...
            if buttons == 'OK':
                ...
                if new_col == 'Green':
                    ...
                    self.light_sq_colour = '#D7CAC1'
                if new_col == 'Wood':
                    self.dark_sq_colour = '#9D7E68'
                    self.light_sq_colour = '#D7CAC1'
                if new_col == 'Pink':
                    self.dark_sq_colour = '#FFC0CB'
                    self.light_sq_colour = '#F5F5DD'
                if new_col == 'Black':
                    self.dark_sq_colour = '#645452'
                    self.light_sq_colour = '#FFFFFF'
