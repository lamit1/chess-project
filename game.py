...
class Game:
    ...
    def highlight_possible_moves(self, window, current_piece, move_from):
        ...
        for sq in move_list:                                                # Loop each square in move_list
            new_elem = window.find_element(key=sq)                           # Get current square element
    ...
    def move_castling(self, window, current_move, turn):
        ...
        :param turn: Engine or Player turn.
        """
        is_white = True if ((self.is_player_white and turn == 'player') or (not self.is_player_white and turn == 'engine')) else False
    ...
    def move_promoting(self, window, current_move, turn):
        is_white = True if ((self.is_player_white and turn == 'player') or (not self.is_player_white and turn == 'engine')) else False
        if is_white:
            # print(1.0)
            self.board_array[1][ord(current_move[0])-ord('a')] = constants.BLANK
            self.board_array[0][ord(current_move[2])-ord('a')] = constants.QUEEN_W
        else :
            # print(2.0)
            self.board_array[6][ord(current_move[0])-ord('a')] = constants.BLANK
            self.board_array[7][ord(current_move[2])-ord('a')] = constants.QUEEN_B
    ...
    def move_handler(self, window, board, player, move, original_sq, new_sq, current_piece):
        ...
        if move.promotion:
            move_str = str(move)
            # print(move_str)
            self.move_promoting(window, move_str, player)
        elif board.is_castling(move):                                              # Check if move is Castling
            move_str = str(move)
        ...
        while is_human_turn:                                                # Loop until turn has moved
            ...
            if type(button) is tuple:                                                 # If chess_board button is clicked
                if not has_user_moved:
                    ...
                    button_element = window.find_element(key=(origin_row, origin_col))
                    ...
                    elif chess.Move.from_uci(str(player_move)+"q") in board.legal_moves:
                        player_move = player_move = chess.Move(origin_square, new_square,chess.QUEEN)
                        self.move_handler(window, board, 'player', player_move, original_sq, new_sq, current_piece)
